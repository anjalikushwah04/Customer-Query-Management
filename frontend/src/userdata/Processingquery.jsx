...
const Processingquery = () => {
  ...
  const pendingquery = async () => {
    const response = await fetch(`http://localhost:8000/processingquery`)
    if (response.ok) {
          ...
          <div className="col-10 scroll h-100 bg-success bg3">
            <div className="row ms-5 h-100 ">
              <div className="col-md-6 mx-auto my-auto text-white">
                <h2 className='text-center mt-4'> Processing Query</h2>
                <table className='table blur4 mt-5 text-center text-dark'>
                  ...
                  <tbody>
                    {proquery?.map((e, i) => (
                      <tr key={i}>
                        ...
                        <td>{e.department}</td>
                        <td>{e.subject}</td>
                        <td><Link to={`/viewprocessingquery/${e._id}`} className='btn btn-primary'>View</Link></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
...
export default Processingquery